...
public class CCC {
	...
	private static final Pattern CCC_REGEX =
			Pattern.compile("(\\d{4})[.-]?(\\d{4})[.-]?([0-9 ])([0-9 ])[.-]?(\\d{10})");
	...
	public CCC(String source) throws Exception {
		...
		if (m.matches())
		{
			...
			//Si los dígitos de control están en blanco, completarlos con lo calculado
			if (! Character.isDigit(this.dc1.charAt(0)))
				this.dc1=String.valueOf(dc1);
			if (! Character.isDigit(this.dc2.charAt(0)))
				this.dc2=String.valueOf(dc2);

			//Si los dígitos de control no coinciden, error
